...
class LockableList:
    ...
    def __setitem__(self, i, values):
        ...
        if isinstance(i, int):
            ...
            if i < 0 or i >= len(self.values):
                raise IndexError("LockableList is out of index range")
            ...
            if step != 1:
                if len(rng) != len(values):
                    raise ValueError(
                        "Attempt to assign a sequence of size{} to extended slice of size {}".format(len(values),
                            len(rng))
                    )
                else:
                ...
                self.value = self.values[:start] + values + self.values[stop:]
        else:
            invalid_type = type(i)
            raise TypeError(
                "LockableList indices must be integers or slices, not {}"
